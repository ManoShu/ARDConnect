...
const uint8_t DEFAULT_POLL_RATE = 30;
...
void setup()
{
  ...
  _pollInterval = 1000 / DEFAULT_POLL_RATE;
}
    ...
    if (deltaTime > _pollInterval)
    {
      if (_inputCount > 0)
      {
        ...
        for (auto i = 0; i < _inputCount; i++)
        {
          ...
          auto currentValue = _inputs[i].digital ? _inputs[i].debouncer->read() : analogRead(_inputs[i].pin);

          if (_inputs[i].value != currentValue)
          {
            auto strValue = _inputs[i].digital ? (currentValue == HIGH ? "H" : "L") : String(currentValue);

            Serial.print(_inputs[i].name + "|" + strValue + "\n");
...
void setPollRate(String message)
{
  ...
  auto strPollRate = getPart(message, 1);

  if (strPollRate != "")
  {
    _pollInterval = 1000 / strPollRate.toInt();
  }
}
